...
int fail(const std::string& message) {
    std::cerr << "[convert-graph-assembly-instance] " << message << '\n';
    return 1;
}
...
std::unique_ptr<CompilationBundle> compileInput(const std::filesystem::path& sourcePath,
    ...
    argStorage.emplace_back("convert-graph-assembly-instance");
...
int testGraphAssemblyInstance(const std::filesystem::path& sourcePath) {
    ...
    const grh::ir::Graph* graph = netlist.findGraph("graph_assembly_instance");
    if (!graph) {
...
int main() {
    const std::filesystem::path sourcePath =
        WOLF_SV_CONVERT_GRAPH_ASSEMBLY_INSTANCE_DATA_PATH;
