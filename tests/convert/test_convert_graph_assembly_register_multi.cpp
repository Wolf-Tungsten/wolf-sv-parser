...
int fail(const std::string& message) {
    std::cerr << "[convert-graph-assembly-register-multi] " << message << '\n';
    return 1;
}
...
std::unique_ptr<CompilationBundle> compileInput(const std::filesystem::path& sourcePath,
    ...
    argStorage.emplace_back("convert-graph-assembly-register-multi");
...
int testGraphAssemblyRegisterMulti(const std::filesystem::path& sourcePath) {
    ...
    const grh::ir::Graph* graph = netlist.findGraph("graph_assembly_register_multi");
    if (!graph) {
...
#ifndef WOLF_SV_CONVERT_GRAPH_ASSEMBLY_REGISTER_MULTI_DATA_PATH
#error "WOLF_SV_CONVERT_GRAPH_ASSEMBLY_REGISTER_MULTI_DATA_PATH must be defined"
#endif

int main() {
    const std::filesystem::path sourcePath =
        WOLF_SV_CONVERT_GRAPH_ASSEMBLY_REGISTER_MULTI_DATA_PATH;
