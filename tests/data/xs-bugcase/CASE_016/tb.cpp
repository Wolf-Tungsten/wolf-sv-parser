...
static int compare_step(const VRef *ref, const VWolf *wolf, int cycle) {
    if (ref->io_fromBpu_prediction_ready_o != wolf->io_fromBpu_prediction_ready_o) {
        std::fprintf(stderr, "[MISMATCH] cycle=%d io_fromBpu_prediction_ready ref=%u wolf=%u\n",
                     cycle, ref->io_fromBpu_prediction_ready_o, wolf->io_fromBpu_prediction_ready_o);
        return 1;
    }
    return 0;
}
