...
namespace wolf_sv_parser::transform
{
    ...
    {
        template <typename T>
        std::optional<T> getAttr(const grh::ir::Operation &op, std::string_view key)
        {
        ...
        bool hasAnyInitAttrs(const grh::ir::Operation &op)
        {
    ...
    PassResult MemoryInitCheckPass::run()
    {
        ...
        for (const auto &entry : netlist().graphs())
        {
            ...
            for (const auto opId : graph.operations())
            {
                ...
                if (op.kind() != grh::ir::OperationKind::kMemory)
                {
                    continue;
                }
