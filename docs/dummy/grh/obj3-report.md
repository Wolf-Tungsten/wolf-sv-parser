...
## 完成事项
- 新增 `wolvrix::lib::grh::SymbolId`/`SymbolTable`，以及 `DesignSymbolTable`/`GraphSymbolTable`，提供 `intern/lookup/contains/text/valid` API，invalid 约定为 0。
...
- 逐步将跨图引用统一切换到 DesignSymbolTable 管理的 `SymbolId`。***
