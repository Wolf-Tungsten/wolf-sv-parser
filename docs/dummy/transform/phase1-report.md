...
## 完成内容
- 为 Value/Operation/Graph/Design 引入 symbol + 指针双轨引用：`definingOpSymbol/Ptr`、`ValueUser.operationSymbol/Ptr`、操作数/结果的指针缓存，并提供 `rehydratePointers()` 在 move/JSON parse 后统一重建。
