...
本文档面向项目贡献者与集成方，概述 wolvrix 的定位、输入输出契约、核心处理流程以及配套支撑能力，作为后续设计文档与实现的入口。

## 项目目标

wolvrix 致力于将高层次行为化的 RTL（基于 Verilog-2005/SystemVerilog 子集）转换为结构化的 GRH 表示，再由 GRH 表示输出成结构化的 verilog 设计，支持可配置的逻辑变换及插件扩展能力，服务于后续仿真、综合、形式验证与定制分析流程。
...
- **结构化设计**：输出为纯组合/时序结构的 Structural Verilog，寄存器、存储器等映射为标准原语实例。
- **层级保持**：保留来源设计的模块层次，黑盒模块直接复用输入端口与参数定义。
- **文件布局**：默认生成单文件 `out.v`，支持通过配置拆分多文件（如按模块层级分组）并自定义输出目录。
- **格式风格**：使用内部生成器的默认格式，不强制额外的格式化工具。
- **元数据产出**：设计生成时同步导出 JSON 文件，记录节点/边统计、各 Pass 的变换摘要及告警信息，为后续工具链提供结构化数据接口。
...
- **测试策略**：
  ...
  - 端到端测试确保代表性 RTL 在输入到设计的转换链路中保持功能等价，并验证黑盒透传。
  - 筛选并维护基于开源 RTL 的基准集合，结合黄金设计或仿真结果进行回归测试；当前尚无官方参考测试集，需要项目内自建。
