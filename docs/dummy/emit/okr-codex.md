...
- **KR1 对齐：文件与构建骨架**
  ...
  - 约定对外接口（如 `Emit::emit(const grh::Netlist&, const StoreOptions&)`）在头文件声明、源文件提供最小默认实现或纯虚钩子，保证后续派生类落地时无需调整主入口。
...
- **KR2 对齐：基类职责与上下文设计**
  ...
  - `Emit` 基类需包含：输入 `grh::Netlist` 的只读视图、诊断收集器、文件/流打开辅助函数、顶层存在性与端口合法性基础校验；对派生类暴露受保护的 `storeImpl` 纯虚接口。
...
- **KR2 对齐：StoreJson 实现与集成**
  - 新增 `StoreJson`（头在emit.hpp/源文件在emit.cpp，命名空间 emit），继承基类并实现 `storeImpl`：接受 `Netlist` 和顶层列表，输出到指定目录（默认 `grh.json`），支持 `jsonMode` 以切换 compact/prettyCompact/pretty。
  - 将历史的 `toJsonString` 入口迁移/薄包装到 `StoreJson`，避免网表类直接持有序列化逻辑，减少重复实现。
  - 更新 CLI `--store-json` 和内部调用路径，改用 `StoreJson`，确保诊断透传（如缺少 top 时提示）并保持现有行为兼容。
- **KR3 对齐：测试与回归覆盖**
  - 在 `tests/emit` 增加针对 `StoreJson` 的单元测试：1）空/无顶层报错；2）单模块最小网表输出、校验关键字段与 key 简写；3）pretty/compact 差异。
  - 回归现有依赖 JSON 的测试（例如 `tests/grh/test_grh.cpp` 写文件路径、elaborate 阶段 JSON dump），迁移到新接口或更新期望输出，确保 ctest 全通过。
  - 在 `tests/store` 测试内添加最小网表与期望 JSON 片段（可内嵌字符串），用于格式断言；确保 CMake 将 StoreJson 测试纳入默认测试集。
- **验收口径**
...
- **KR3 对齐：样例与 lint 验证**
  - 在 `tests/emit` 内嵌覆盖多段的最小网表示例（含端口/wire/reg/mem/实例/DPIC/组合/时序），并提供期望 `.sv` 片段或快照。
