...
- **属性与状态校验（KR2）**：Finalize 过程中检查寄存器 state op 是否尚未被其他块占用、`rstLevel` / 输出位宽是否与 memo 匹配，如有冲突立即通过 `reportFinalizeIssue` 抛出诊断，避免静默连线。
...
- **测试与可视化**：`tests/data/elaborate/seq_always.sv` 扩充了普通寄存器、分段赋值、同步复位、异步复位四类 always_ff；`tests/elaborate/test_elaborate_seq_always.cpp` 校验 op kind、operand 顺序、复位 mux、slice/concat 结构以及 `rstLevel` 属性，`ctest -R elaborate-seq-always` 作为阶段回归。
...
3. **复位值可视化**：测试与 artifact 中加入 reset 逻辑的 JSON dump（例如标出 mux 结构、rstLevel、resetValue Literal），方便后续调试更复杂的 reset 场景。
