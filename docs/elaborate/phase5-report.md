...
## Objective 进展
- Elaborate 现已在模块级别解析 slang AST 中的 `wire/reg/logic` 声明，生成独立的 wire/reg 备忘录（memo），供后续 pass 直接 peek。
- 为所有候选信号收集驱动来源（连续赋值、`always_*`/`initial` 等），根据时序特性自动分类为 wire/reg，冲突时立即发出诊断并从 memo 中剔除。
...
- `Elaborate` 内部维护 `wireMemo_/regMemo_` 两个 `unordered_map<InstanceBodySymbol*, vector<SignalMemoEntry>>`，以模块体 (`InstanceBodySymbol`) 为 key，确保 canonical body 与实例 body 可复用缓存。
- 提供 `peekWireMemo` / `peekRegMemo` 公共接口，返回 `std::span<const SignalMemoEntry>`，测试与后续 pass 均可零拷贝读取。

## KR2 声明遍历与候选收集
- 在 `collectSignalMemos` 中遍历模块成员列表，对 `NetSymbol`、`VariableSymbol`（含 logic）进行筛选，仅保留 `type.isIntegral()` 的信号。
- 使用 `TypeHelper::analyze` 计算位宽/符号属性，保证 memo 与端口/Value 建模保持一致。
...
- 时序分类：
  - `always_comb` / `always @(*)` → wire；
  - `always_ff`、含 `posedge/negedge` 的 `always`、`initial`/`final`、`always_latch` → reg；
  - `always @(implicit)` → wire；
  - `containsEdgeSensitiveEvent` 递归解析 `TimingControl`（`SignalEvent/EventList/RepeatedEvent`）判断是否包含边沿事件。
- 使用 `MemoDriverKind` 位掩码 (Wire/Reg) 记录每个符号的驱动集合，若同一符号同时出现 wire/reg 驱动，立即下发 `NYI` 诊断并在最终 memo 中剔除该信号。

## KR4 Memo 写入与排序
- 将仅由 wire 驱动的候选写入 wire memo，仅由 reg 驱动的候选写入 reg memo；其他情况（未驱动或冲突）保持忽略。
...
- 新增 `tests/data/elaborate/signal_memo.sv`，覆盖：
  ...
  - 同一信号被 wire/reg 双重驱动的冲突场景。
- 新测 `tests/elaborate/test_elaborate_signal_memo.cpp`：
  - 通过 `Elaborate::peekWireMemo/peekRegMemo` 验证 `w_assign`、`comb_bus`（宽度 8、signed）、`star_assign` 属于 wire memo；
