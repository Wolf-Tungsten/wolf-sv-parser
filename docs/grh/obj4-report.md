...
## 完成事项
- 新增 `grh::ir::GraphView` 最小只读接口，覆盖 `operations/values/opKind/opOperands/opResults/valueWidth/valueDef/valueUsers`。
- 新增 `grh::ir::GraphBuilder`，提供 `addValue/addOp/addOperand/addResult` 与 `freeze()` 压平生成只读视图。
