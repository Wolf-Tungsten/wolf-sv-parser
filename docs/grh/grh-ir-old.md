...
高层次行为化的 SystemVerilog - slang AST - **GRH IR** - 低层次结构化的 SystemVerilog 设计
...
- GRH IR API 以 `SymbolId` 作为符号句柄，通过 `GraphSymbolTable`/`DesignSymbolTable` 驻留字符串
...
- 端口名为普通字符串，不进入 `GraphSymbolTable` 或 `DesignSymbolTable`
...
- 具有一个 graph symbol（module 名称），符合 verilog 标识符规范，在设计中唯一，并由 `DesignSymbolTable` 分配 `GraphId`；JSON load/emit 缺失 graph symbol 视为错误
- 具有一个 `GraphSymbolTable` 管理图内符号，`Graph::internSymbol/lookupSymbol/symbolText` 为主要入口（仅用于 value/op/declaredSymbols）
- 端口分为 input/output/inout：input/output 以 `std::vector<Port>` 存储，元素为 `{std::string name, ValueId value}`；inout 以 `std::vector<InoutPort>` 存储，元素为 `{std::string name, ValueId in, ValueId out, ValueId oe}`；`bindInputPort/bindOutputPort/bindInoutPort` 负责绑定
    ...
    ${CommaSeparatedList(
        for (const auto& port : inputPorts)
            -> "input "  + (value(port.value).signed ? "signed " : "")
               + "[" + std::to_string(value(port.value).width - 1) + ":0] " + port.name,
        for (const auto& port : outputPorts)
            -> "output " + (value(port.value).signed ? "signed " : "")
               + "[" + std::to_string(value(port.value).width - 1) + ":0] " + port.name,
        for (const auto& port : inoutPorts)
            -> "inout "  + (value(port.out).signed ? "signed " : "")
               + "[" + std::to_string(value(port.out).width - 1) + ":0] " + port.name
    )}
);
    ${for (const auto& port : inoutPorts)
        -> "assign " + valueSymbol(port.in) + " = " + port.name + ";"
           "assign " + port.name + " = " + valueSymbol(port.oe) + " ? "
...
其中 `value(...)` 表示通过 Graph 访问 Value 元数据；`CommaSeparatedList` 依次枚举 inputPorts、outputPorts、inoutPorts；在冻结后的 `GraphView` 中，端口按端口名的字典序排序（构建期插入顺序不保证保持原始声明顺序）。
...
- 端口 `name` 为普通字符串，不要求在 `GraphSymbolTable` 中存在
...
- 具有一个可按 graph symbol 索引 Graph 的容器，要求 symbol 在设计中唯一，并保留 `graphOrder` 记录创建顺序
- Graph 名称由 `DesignSymbolTable` 驻留并分配 `GraphId`；Graph 内符号与 Design 符号表相互独立
- 顶层模块由 `markAsTop()` 维护 `topGraphs` 列表，不再依赖 Graph 内部标记
- instance Operation 的 attributes 中记录被例化模块的 moduleName，运行时通过 Design 解析被例化 Graph；禁止跨设计引用
- 允许存在未被引用的 Graph（如库模块），但综合或导出流程默认仅从顶层 Graph 开始遍历可达子图
- 别名通过 `registerGraphAlias(alias, graph)` 维护，`findGraph` 会解析 alias
- 层次结构由 Graph 内的 instance Operation 建模，设计自身不额外存储层次边
...
- attributes：
    - moduleName（string）：被实例化模块的名称，在 design 中必须能通过 moduleName 找到一个 graph
...
- `targetImportSymbol` 必须在当前 Design 中解析到唯一的 kDpicImport；
...
构图或变换流程在处理 `kDpicCall` 时，需使用 `targetImportSymbol` 字符串到 Design 中解析出对应的 `kDpicImport` Operation，并从该 Operation 的 attributes 中读取形参方向、位宽与名称等信息；若解析失败或发现多个候选项，必须立即报错。下文伪代码中的 `importOp` 表示解析得到的 kDpicImport Operation，`importOp.argsName` 等字段均来自该 Operation 的 attributes。
