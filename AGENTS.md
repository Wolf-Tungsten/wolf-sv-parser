...
- Core sources live in `src/` with headers in `include/`; the CLI entry is `src/main.cpp` linking `grh`, `convert`, `transform`, and `emit`.
- Tests sit under `tests/{grh,convert,transform,emit}` with fixtures in `tests/data` and test-only artifacts written to `build/artifacts` (created by CMake).
- Source files:
  - `src/grh.cpp` - GRH (Graph RTL Hierarchy) implementation
  - `src/convert.cpp` - Convert SystemVerilog AST to GRH
  - `src/transform.cpp` - Transformation passes (constant folding, dead code elimination, etc.)
  - `src/emit.cpp` - Emit GRH to SystemVerilog or JSON
  - `src/load.cpp` - Load GRH from JSON
...
- Manual run example: `./build/bin/wolf-sv-parser --emit-sv --emit-json -o out.sv path/to/file.sv`.
...
## Testing Guidelines
- Tests are standalone executables registered via CTest; they fail by returning non-zero. Place new cases alongside their module (e.g., `tests/convert/`).
- Name targets descriptively (`emit-json`, `convert-symbol-collector`, etc.) and wire fixture paths through `target_compile_definitions` in `CMakeLists.txt`.
