...
- Source files:
  ...
  - `lib/src/transform.cpp` - Transformation passes (constant folding, dead code elimination, etc.)
  - `lib/transform/*.cpp` - Built-in transform passes
  - `lib/src/emit.cpp` - Emit GRH to SystemVerilog
  - `lib/src/store.cpp` - Store GRH as JSON
  - `lib/src/load.cpp` - Load GRH from JSON
...
- Build: `cmake --build build -j$(nproc)`; resulting binary is `build/bin/wolvrix`.
...
- Manual run example: `./build/bin/wolvrix -c "read_sv path/to/file.sv --top top; write_sv -o out.sv"`.
...
- Store fixtures in `tests/<module>/data` (shared suites stay in `tests/data/...`); write artifacts only under `build/artifacts` or `build/hdlbits` to keep the tree clean.
